...
public class LineIterator implements Iterator<String> {
	...
	/**
	 * 标识输入流中是否还有行可供读入，如果程序产生了<code>IOException</code>， close()将会被调用，以关闭输入流，并抛出
	...
	/**
	 * 关闭<code>Reader</code> 如果你只想读取一个大文件的头几行，那么这个函数可以 帮助你关闭输入流。如果没有调用close函数，那么
	 * <code>Reader</code>将保持打开的状态。这一方法可以 安全地多次调用。
	 */
	public void close() {
		finished = true;
		try {
