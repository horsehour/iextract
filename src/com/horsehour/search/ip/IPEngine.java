...
/**
 * search engine for ip information
 * 
...
public class IPEngine {
	...
	private Map<String, IPLocation> cacheTable;
	...
	public IPEngine(String dbFile) {
		ipDB = dbFile;
		cacheTable = new HashMap<String, IPLocation>();
		loc = new IPLocation();
		buf = new byte[100];
		...
		try {
			ipFile = new RandomAccessFile(ipDB, "r");
			if (ipFile == null)
				return;
			// if succeed, read header information
			ipBegin = readLong4(0);
			ipEnd = readLong4(4);
	...
	public List<IPEntry> getIPEntriesDebug(String s) {
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			IPLocation loc = retrieveLocation(temp);

			String country = loc.getCountry();
			String area = loc.getArea();
			if (country.contains(s) || area.contains(s)) {
	...
	/**
	 * retrieve location of given ip
	 * 
	 * @param ip
	 * @return location of given ip
	 */
	...
	public String getCountry(byte[] ip) {
		...
		if (cacheTable.containsKey(ipStr)) {
			ipLoc = cacheTable.get(ipStr);
		} else {
			ipLoc = getIPLocation(ip);
			cacheTable.put(ipStr, ipLoc.copy());
	 ...
	 * @return IPLocation对象
	 */
	private IPLocation retrieveLocation(long offset) {
		try {
			// 跳过4字节ip
	 ...
	 * @return IPLocation对象
	 */
	private IPLocation retrieveLocation(int offset) {
		// 跳过4字节ip
